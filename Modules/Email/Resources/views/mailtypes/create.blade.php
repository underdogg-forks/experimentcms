...
@section('content-header')
    <h1>
        {{ trans('email::mailtypes.title.create mailtype') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.email.mailtype.index') }}">{{ trans('email::mailtypes.title.mailtypes') }}</a></li>
        <li class="active">{{ trans('email::mailtypes.title.create mailtype') }}</li>
    </ol>
@stop
...
@section('content')
    {!! Form::open(['route' => ['admin.email.mailtype.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('email::admin.mailtypes.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
