...
namespace Modules\Auth\Sidebar;
...
class SidebarExtender implements \Maatwebsite\Sidebar\SidebarExtender
{
    ...
    public function extendWith(Menu $menu)
    {
        $menu->group(trans('workshop::workshop.title'), function (Group $group) {
            $group->item(trans('auth::users.title.users'), function (Item $item) {
                $item->weight(0);
                ...
                $item->item(trans('auth::users.title.users'), function (Item $item) {
                    $item->weight(0);
                    $item->icon('fa fa-users');
                ...
                $item->item(trans('auth::roles.title.roles'), function (Item $item) {
                    $item->weight(1);
        ...
        $menu->group(trans('auth::users.my account'), function (Group $group) {
            $group->weight(110);
            $group->item(trans('auth::users.profile'), function (Item $item) {
                $item->weight(0);
                $item->icon('fa fa-user');
                $item->route('admin.account.profile.edit');
            });
            $group->item(trans('auth::users.api-keys'), function (Item $item) {
                $item->weight(1);
                $item->icon('fa fa-key');
