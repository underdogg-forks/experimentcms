...
@section('content-header')
    <h1>
        {{ trans('email::mailboxtypes.title.create mailboxtype') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.email.mailboxtype.index') }}">{{ trans('email::mailboxtypes.title.mailboxtypes') }}</a></li>
        <li class="active">{{ trans('email::mailboxtypes.title.create mailboxtype') }}</li>
    </ol>
@stop
...
@section('content')
    {!! Form::open(['route' => ['admin.email.mailboxtype.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('email::admin.mailboxtypes.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
