...
@section('title')
    {{ trans('auth::auth.register') }} | @parent
@stop
...
@section('content')
    ...
    <div class="register-box-body">
        <p class="login-box-msg">{{ trans('auth::auth.register') }}</p>
        @include('partials.notifications')
        {!! Form::open(['route' => 'register.post']) !!}
            <div class="form-group has-feedback {{ $errors->has('email') ? ' has-error has-feedback' : '' }}">
                <input type="email" name="email" class="form-control" autofocus
                       placeholder="{{ trans('auth::auth.email') }}" value="{{ old('email') }}">
            ...
            <div class="row">
                <div class="col-xs-12">
                    <button type="submit" class="btn btn-primary btn-block btn-flat">{{ trans('auth::auth.register me') }}</button>
                </div>
            </div>
        {!! Form::close() !!}

        <a href="{{ route('login') }}" class="text-center">{{ trans('auth::auth.I already have a membership') }}</a>
    </div>
@stop
